...
exports.initialize = function initializeSchema(schema, callback) {
  ...
  var config = {
    ...
    options: schema.settings.options
  };
      ...
      if (hasOptions) {
        if (typeof options == "object" && Array.isArray(options)) {
          for (var i = 0; i < options.length; i++) {
            request.input(options[i].name, options[i].value);
          }
        }
        else {
  ...
  Object.keys(data).forEach(function (key) {
    if (props[key]) {
      ...
      if (key !== "id" && key !== modelPKID) {
        fieldNames.push("[" + key + "]");
        fieldValuesPlaceholders.push("(@" + key + ")");
        combined.push(key + "=(@" + key + ")");
        params.push({ name: key, value: self.toDatabase(props[key], data[key]) })
      }
    }
  });
  var sql = "";
...
MsSQL.prototype.exists = function (model, id, callback) {
  ...
  //console.log(sql);
  this.query(sql, [{ name: "id", value: id }], function (err, results) {
      if (err) return callback(err);
...
MsSQL.prototype.destroy = function(model, id, cb) {
  ...
  this.query(sql, [{ name: "id", value: id }], function(err, data) {
    if (err) { return cb(err); }
    cb(null);
  });
};
...
MsSQL.prototype.buildInsert = function (model, data) {
  ...
  Object.keys(data).forEach(function (key) {
    if (props[key]) {
      ...
      paramPlaceholders.push("(@" + key + ")");
      params.push({ name: key, value: this.toDatabase(props[key], data[key])});
    }
  }.bind(this));
...
MsSQL.prototype.all = function (model, params, callback) {
  ...
  if (params) {
    if (params.where) {
      sql += this.buildWhere(params.where, props) + MsSQL.newline;
      //console.log(sql);
    }

    if (params.limit || params.skip) {
        var pk = Object.keys(props).filter(function (path) {return props[path].primaryKey;})[0];
        if (!params.order && pk) params.order = pk;
    }
  ...
  this.query(sql, function (err, data) {
    ...
    if (params && params.include) {
        self._models[model].model.include(data, params.include, callback);
    } else {
      callback(null, data);
    }
  });
